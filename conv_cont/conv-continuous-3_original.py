...
class Conv(GraphScene):
    ...
    def draw_x_t(self,y0):
        ...
        x1 = Line([-8, y0, 0], [2, y0, 0])
        x1.set_color(GREEN)
        x2 = Line([2, y0, 0], [2, 1 + y0, 0])
        x2.set_color(GREEN)
        x3 = Line([2, 1 + y0, 0], [4, 1 + y0, 0])
        x3.set_color(GREEN)
        x4 = Line([4, 1 + y0, 0], [4, y0, 0])
        x4.set_color(GREEN)
        x5 = Line([4, y0, 0], [8, y0, 0])
        x5.set_color(GREEN)
        #
    ...
    def draw_h_t(self, y0):
        ht = VGroup()
        h1 = Line([-8, y0, 0], [0, y0, 0])
        h1.set_color(BLUE)
        h2 = Line([0, y0, 0], [1, 2 + y0, 0])
        h2.set_color(BLUE)
        h3 = Line([1, 2 + y0, 0], [1,  y0, 0])
        h3.set_color(BLUE)
        h4 = Line([1, y0, 0], [8, y0, 0])
        h4.set_color(BLUE)
        
        ht.add(h1,h2,h3,h4,h4)
    ...
    def ht_reflect(self,y0,ht, htau_text):
        ...
        hm2.set_color(BLUE)
        hm3 = Line([-1, 2 + y0, 0], [0, y0, 0])
        hm3.set_color(BLUE)
        hm4 = Line([0, y0, 0], [5, y0, 0])
        hm4.set_color(BLUE)
        
        hmtau.add(hm1, hm2, hm3, hm4)
    ...
    def moving_window_intro(self,y0,hmtau_text):
        ...
        htmtau_text = TextMobject("""
            $$h(t-\\tau)= (2t - 2\\tau) [u(t-\\tau) - u(t-\\tau-1)]$$
        """)
        ...
        sup_limit_text.move_to([0, y0-0.6, 0])
    ...
    def construct(self):
        ...
        # Pantalla Inicial
        self.titulo_credits("Convolución Contínua","Ejemplo 3","Ing Leandro Borgnino TSSL UNC 2023")
        ...
        y1 = Line([-8, yz, 0], [2, yz, 0])
        y1.set_color(ORANGE)
        y2 = self.get_graph(lambda x : x**2-4*x+4, x_min=2, x_max=3)
        y2.move_to([2+0.5, yz+0.5, 0])
        #FunctionGraph(
        #    lambda t: np.sin(t) + 0.5 * np.sin(7 * t) + (1 / 7) * np.sin(14 * t),
        #    x_range=[0, 0],
        #    color=GREEN,
        #).move_to([0, 1, 0])
        #FunctionGraph(lambda x : x**2-4*x+4, x_range=[0,1])#.move_to([2,0+yz,0])#Line([2, 0 + yz, 0], [3, 2 + yz, 0])
        y2.set_color(ORANGE)
        y3 = Line([3, 1+yz, 0], [4, 1+yz, 0])
        y3.set_color(ORANGE)
        y4 = self.get_graph(lambda x : -(x**2)+8*x-15, x_min=4, x_max=5)
        y4.move_to([4+0.5, yz+0.5, 0])
        y4.set_color(ORANGE)
        y5 = Line([5, 0 + yz, 0], [7,  0 + yz, 0])
        y5.set_color(ORANGE)
        #
        #
        ## dots 1
        ...
        self.play(
            window.shift, [7, 0, 0],
            sup_limit_text.shift, [7, 0, 0],
            inf_limit_text.shift, [7, 0, 0],
            t_value.set_value, 2,
            ShowCreation(y1),
            rate_func=linear,
        ...
        one = self.get_graph(lambda x: 2, x_min=2, x_max=3)
        area = self.color_area(one, 2, 3)
        ...
        second_interval_y = TextMobject("""
            $$ y(t) = \int_{2}^{t} (2t-2\\tau) d \\tau = t^2 - 4t + 4$$
        """)
        ...
        third_interval_title.move_to([3.5, 3.5+0.2, 0])
        third_interval_title.scale(0.5)       

        third_interval_t = TextMobject("""
            $$ 3 < t \leq 4$$
        """)
        third_interval_t.set_color(ORANGE)
        third_interval_t.move_to([3.5, 3+0.2, 0])
        third_interval_t.scale(0.6)       

        third_interval_y = TextMobject("""
            $$ y(t) = \int_{t-1}^{t} (2t - 2\\tau) d \\tau = 1 $$
        """)
        third_interval_y.set_color(ORANGE)
        third_interval_y.move_to([3.5, 2.0+0.3, 0])
        third_interval_y.scale(0.6)
        ...
        fourth_interval_title.move_to([3.5, 3.5+0.3, 0])
        fourth_interval_title.scale(0.5)       

        fourth_interval_t = TextMobject("""
            $$ 4 \leq t < 5 $$
        """)
        fourth_interval_t.set_color(ORANGE)
        fourth_interval_t.move_to([3.5, 3+0.3, 0])
        fourth_interval_t.scale(0.6)       

        fourth_interval_y = TextMobject("""
            $$ y(t) = \int_{t-1}^4 (2t-2\\tau) d \\tau = -t^2 + 8t - 15 $$
        """)
        fourth_interval_y.set_color(ORANGE)
        fourth_interval_y.move_to([3.5, 2.0+0.4, 0])
        fourth_interval_y.scale(0.6)
        
        self.play(Write(fourth_interval_t), Write(fourth_interval_y), Write(fourth_interval_title))
        #
        #self.play(FadeOut(area))
        one = self.get_graph(lambda x: 2, x_min=3, x_max=4)
        area2 = self.color_area(one, 3, 4, opacity=2.0)
